  ...
  try {
    ...
    try {
      ...
      if (fileListener->start(baseAddr)) {
        ...
        LOG_INFO("file transfer listener started on port 24802");
      } else {
        LOG_WARN("failed to start file transfer listener, will use main connection");
        delete fileListener;
      }
    } catch (BaseException &e) {
      LOG_WARN("exception starting file transfer listener: %s", e.what());
    }
